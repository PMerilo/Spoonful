...
namespace Spoonful.Services
{
    public class ShoppingListService
    {
        ...
        {
            return _context.Shopping.Where(m => m.Category.ToLower() == userId).OrderBy(m => m.Id).ToList();
        }
        public List<ShoppingEntry> GetAllByName(string userId)
        {
            return _context.Shopping.Where(m => m.Category.ToLower() == userId).OrderBy(m => m.Name).ToList();
        }
        public List<ShoppingEntry> GetAllByCat(string userId)
        {
            return _context.Shopping.Where(m => m.Category.ToLower() == userId).OrderBy(m => m.Category).ToList();
        }   
        public List<ShoppingEntry> GetAllByPurchase(string userId)
        {
            return _context.Shopping.Where(m => m.Category.ToLower() == userId).OrderBy(m => m.isBought).ThenBy(m => m.Category).ToList();
        }
